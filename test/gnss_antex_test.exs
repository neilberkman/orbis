defmodule Orbis.GNSS.AntexTest do
  use ExUnit.Case
        ...
        for sample <- frequency_golden["pcv_samples_mm"] do
          ...
          value =
            if is_nil(sample["azimuth_deg"]) do
              Antex.pcv(antenna, frequency, zenith)
            else
              Antex.pcv(antenna, frequency, zenith, sample["azimuth_deg"])
            end
  ...
  test "pcv interpolates linearly in zenith", %{
    ...
    assert_in_delta Antex.pcv(antenna, frequency, zenith_mid), expected, 1.0e-12
    assert_in_delta Antex.pcv(antenna, frequency, -10.0), hd(noazi).value_m, 1.0e-12
    assert_in_delta Antex.pcv(antenna, frequency, 999.0), List.last(noazi).value_m, 1.0e-12
  end
  ...
  test "pcv interpolates azimuth with wrap", %{antex: antex} do
    ...
    assert_in_delta Antex.pcv(antenna, frequency, zenith, az_mid), expected, 1.0e-12

    assert_in_delta Antex.pcv(antenna, frequency, zenith, 359.0),
                    Antex.pcv(antenna, frequency, zenith, -1.0),
                    1.0e-12
  end
