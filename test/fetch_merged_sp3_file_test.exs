defmodule Orbis.GNSS.FetchMergedSP3FileTest do
  ...
  describe "Data.fetch_merged_sp3_file/4" do
    setup do
      cache_dir =
        Path.join(
          System.tmp_dir!(),
          "orbis_fetch_merged_sp3_file_#{System.unique_integer([:positive])}"
        )

      out_dir =
        Path.join(
          System.tmp_dir!(),
          "orbis_fetch_merged_sp3_file_out_#{System.unique_integer([:positive])}"
        )
  ...
  describe "Data.fetch_merged_sp3_file/4 (network)" do
    ...
    test "fetches and writes a real merged current-day ultra product" do
      cache_dir =
        Path.join(System.tmp_dir!(), "orbis_fmsf_net_#{System.unique_integer([:positive])}")

      out_dir =
        Path.join(System.tmp_dir!(), "orbis_fmsf_net_out_#{System.unique_integer([:positive])}")
