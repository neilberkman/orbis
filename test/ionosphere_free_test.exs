defmodule Orbis.IonosphereFreeTest do
  ...
  describe "frequency table" do
    ...
    test "frequencies/0 exposes the full table" do
  ...
  describe "end-to-end: iono-free solve beats single-frequency-with-iono" do
    ...
    test "iono-free recovers the receiver; L1-only-with-iono is badly biased" do
      ...
      # Surface the measured numbers for the run report.
      IO.puts(
        "\n[iono-free end-to-end] baseline=#{fmt(base_err)} m  " <>
          "L1-only-with-iono=#{fmt(l1_err)} m  iono-free=#{fmt(if_err)} m"
      )
    end
  end
  ...
  defp err(sol, {tx, ty, tz}) do
    :math.sqrt(
      (sol.position.x_m - tx) ** 2 + (sol.position.y_m - ty) ** 2 + (sol.position.z_m - tz) ** 2
    )
  end

  defp fmt(x), do: :erlang.float_to_binary(x, decimals: 6)
end
