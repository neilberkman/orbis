defmodule Orbis.GNSS.RINEX.ClockTest do
  use ExUnit.Case, async: true

  alias Orbis.GNSS.RINEX.Clock
  ...
  setup do
    path =
      Path.join(System.tmp_dir!(), "orbis_clock_test_#{System.unique_integer([:positive])}.clk")
  ...
  describe "load/1" do
    test "returns an error tuple for a missing file" do
      assert {:error, _reason} = Clock.load("/nonexistent/orbis_clock.clk")
    end
  end
end
