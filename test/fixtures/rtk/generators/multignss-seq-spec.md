...
This is a validate-and-document capability: origin/main (orbis 0.22.0) already
...
- Truth: antenna ARP baseline `rover_arp - base_arp` from the marker ECEF
  ...
  magnitude is frame-invariant, so the Orbis ECEF baseline is compared directly
...
- `orbis_min_sats >= oracle floor` (the harness uses SP3/mask-10, the oracle
