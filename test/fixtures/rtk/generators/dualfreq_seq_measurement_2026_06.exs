defmodule DualFreqSeqMeasurement202606 do
  ...
  defp verdict_paragraph(h, oracle) do
    ...
    case inv["verdict"] do
      ...
      "underpowered" ->
        "Amendment 1 UNDERPOWERED: fixed n = #{inv["fixed_n"]} < #{@min_fixed_n}. Not a pass per the refusal invariant; reported honestly as a null result for this cell."

      "FAIL-by-floor" ->
        "Amendment 1 FAIL-by-floor: fixed median 3D error #{fmt_m(inv["fixed_median_error_m"])} exceeds the #{fmt_m(floor)} floor. Reported honestly."
