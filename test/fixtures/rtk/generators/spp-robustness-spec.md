...
(`:robust`) for `Orbis.GNSS.Positioning.solve/4`. This is the rebuild of the
...
`Orbis.GNSS.QC` `fde_loop/8` cap branch returned `{:ok, ...}` when the
...
each vendored arc, run orbis SPP per matched epoch as (A) bare crate solve,
(B) robust with unit weights via `:unsafe_unit_weights` (the harmful mode,
recorded honestly), and (C) robust with a realistic 5 m uniform phone code sigma
...
horizontal median and p95 vs truth.
  ...
  * Credibility floor: orbis bare 3D median within 2x the demo5 median
    (absolute-accuracy floor only; orbis is unaided single-freq SPP vs tuned
