defmodule Orbis.GnssObservables do
  @moduledoc """
  ...
  transmit time. It reads satellite states only from `Orbis.SP3.position/3` and
  ...
  * **Range rate.** The satellite velocity at `t_tx` is obtained by central
    finite difference of `Orbis.SP3.position/3` (+/- 0.5 s). For a static
  ...
  **negative when the satellite is approaching** (range decreasing) and positive
  ...
  @c 299_792_458.0
  # Earth rotation rate (WGS84 / IS-GPS-200), rad/s.
  ...
  def predict(%SP3{} = sp3, satellite_id, receiver_ecef, %NaiveDateTime{} = epoch, opts \\ [])
    ...
    carrier_hz = Keyword.get(opts, :carrier_hz, @f_l1)
    ...
    with {:ok, {rx, ry, rz}} <- normalize_receiver(receiver_ecef),
      ...
      doppler_hz = -range_rate * carrier_hz / @c
