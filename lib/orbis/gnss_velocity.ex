defmodule Orbis.GnssVelocity do
  @moduledoc """
  ...
  This is the velocity counterpart to `Orbis.PointPositioning`: where single-point
  ...
  `Orbis.GnssObservables` for the satellite geometry and velocity, and the explicit
  4x4 inverse in `Orbis.GnssGeometry.inv4/1` for the normal-equation solution.
  ...
  Every quantity comes from `Orbis.GnssObservables.predict/5` evaluated at the
  ...
  (the convention in `Orbis.GnssObservables`), so with `opts[:observable] = :doppler`
  ...
  # Speed of light in vacuum, m/s (matches Orbis.GnssObservables).
  @c 299_792_458.0
  # IS-GPS-200 L1 carrier frequency, Hz (1575.42 MHz).
  ...
  `source` is a loaded `Orbis.SP3` precise product. `observations` is a list of
  ...
  def solve(%SP3{} = sp3, observations, %NaiveDateTime{} = epoch, receiver_position, opts)
    ...
    carrier_hz = Keyword.get(opts, :carrier_hz, @f_l1) * 1.0
    ...
    with :ok <- ensure_nonempty(observations),
         {:ok, receiver} <- normalize_receiver(receiver_position),
  ...
  convention in `Orbis.GnssObservables` (`doppler_hz = -rho_dot * carrier_hz / c`).
  `carrier_hz` defaults to the L1 carrier.
  """
  @spec doppler_to_range_rate(number(), number()) :: float()
  def doppler_to_range_rate(doppler_hz, carrier_hz \\ @f_l1) do
    -doppler_hz * @c / carrier_hz
  end

  @doc """
  Convert a pseudorange rate in m/s to a Doppler shift in Hz.

  Uses `doppler_hz = -rho_dot * carrier_hz / c`, matching `Orbis.GnssObservables`.
  `carrier_hz` defaults to the L1 carrier.
  """
  @spec range_rate_to_doppler(number(), number()) :: float()
  def range_rate_to_doppler(rho_dot_m_s, carrier_hz \\ @f_l1) do
    -rho_dot_m_s * carrier_hz / @c
  end
  ...
  defp build_rows(sp3, normalized, epoch, receiver, sat_drift_fun, predict_opts) do
    Enum.reduce_while(normalized, {:ok, []}, fn {sat, rho_dot}, {:ok, acc} ->
      case GnssObservables.predict(sp3, sat, receiver, epoch, predict_opts) do
        {:ok, obs} ->
          {ex, ey, ez} = obs.los_unit
          e_dot_vsat = obs.range_rate_m_s
          sat_drift = sat_drift_fun.(sat)
          y = rho_dot - e_dot_vsat + @c * sat_drift
