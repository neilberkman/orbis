defmodule Orbis.GNSS.Troposphere do
  @moduledoc """
  ...
  def mapping(elevation_deg, lat_deg, height_m, epoch) do
    with {jd_whole, jd_fraction} <- Orbis.GNSS.Time.epoch_to_split_jd(epoch) do
      {dry, wet} =
  ...
  def slant_delay(elevation_deg, lat_deg, lon_deg, height_m, met, epoch) do
    with {:ok, p, t, rh} <- meteorology(met),
         {jd_whole, jd_fraction} <- Orbis.GNSS.Time.epoch_to_split_jd(epoch) do
      delay =
