defmodule Orbis.GNSS.Geometry do
  @moduledoc """
  ...
  *after* inverting, by rotating the 3x3 position block into the local ENU frame
  ...
  def visible(%SP3{} = sp3, receiver, %NaiveDateTime{} = epoch, opts \\ []) do
    with {:ok, rx} <- Types.normalize_ecef(receiver) do
      mask = Keyword.get(opts, :elevation_mask_deg, @default_mask_deg)
      systems = Keyword.get(opts, :systems)

      sp3
      |> Observables.predict_all(rx, epoch, light_time: false)
      |> Enum.flat_map(fn
        {sat_id, {:ok, obs}} ->
          if Types.system_allowed?(sat_id, systems) and obs.elevation_deg >= mask do
            [
              %{
                satellite_id: sat_id,
    ...
    with {:ok, rx} <- Types.normalize_ecef(receiver) do
      ...
      rows_weights =
        Enum.flat_map(sats, fn sat_id ->
          case Observables.predict(sp3, sat_id, rx, epoch,
  ...
  def passes(%SP3{} = sp3, receiver, {t0, t1}, step_seconds, opts \\ []) do
    with {:ok, rx} <- Types.normalize_ecef(receiver) do
      ...
      sp3
      |> SP3.satellite_ids()
      |> Enum.filter(&Types.system_allowed?(&1, systems))
      |> Enum.flat_map(fn sat_id ->
        samples =
          Enum.flat_map(epochs, fn epoch ->
            case Observables.predict(sp3, sat_id, rx, epoch, light_time: false) do
