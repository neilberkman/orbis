  ...
  *double difference* subtracts a reference satellite's single difference:
  ...
  Additional wide-lane options:
    ...
    * `:wide_lane_tolerance_cycles` - maximum absolute distance between the
      ...
      cycles).
    * `:narrow_lane_model` - `:ionosphere_free` (default) forms the
      long-baseline ionosphere-free narrow-lane combination. `:l1_direct` keeps
      the short-baseline L1/L2 carrier rows directly after the wide-lane fix,
      using the wide-lane integer to constrain `N2 = N1 - Nw`.
  ...
  def solve_widelane_fixed_baseline_epochs(base_position, dual_epochs, opts)
    ...
    with {:ok, base} <- Types.normalize_ecef(base_position, :invalid_base_position),
         ...
         {:ok, narrow_lane_model} <- dual_narrow_lane_model(opts),
         {:ok, %FixedBaselineSolution{} = sol} <-
           solve_widelane_narrow_lane_model(
             narrow_lane_model,
             base_position,
  ...
  defp dual_narrow_lane_model(opts) do
    case Keyword.get(opts, :narrow_lane_model, :ionosphere_free) do
      model when model in [:ionosphere_free, :l1_direct] -> {:ok, model}
      _other -> {:error, {:invalid_option, :narrow_lane_model}}
    end
  end

  defp solve_widelane_narrow_lane_model(
         :ionosphere_free,
         base_position,
         prepared_dual_epochs,
         reference_sat,
    ...
    with {:ok, if_epochs, wavelengths, offsets} <-
      ...
      solve_fixed_baseline_epochs(base_position, if_epochs, fixed_opts)
    end
  end
  ...
  defp solve_widelane_narrow_lane_model(
    ...
    solve_widelane_l1_direct_baseline(
      base_position,
      prepared_dual_epochs,
      reference_sat,
      wide_lane_cycles,
  ...
  defp solve_widelane_l1_direct_baseline(
         base_position,
         dual_epochs,
         reference_sat,
         wide_lane_cycles,
    ...
    with {:ok, base} <- Types.normalize_ecef(base_position, :invalid_base_position),
         ...
         {:ok, initial_baseline} <- initial_baseline(opts),
         {:ok, integer_opts} <- integer_options(opts),
         {:ok, ambiguity_ids, ambiguity_satellites, physical_sats, params} <-
           dual_l1_direct_ambiguity_index(dual_epochs, reference_sat, wide_lane_cycles) do
      if dual_l1_direct_row_count(dual_epochs, reference_sat, physical_sats, wide_lane_cycles) <
           baseline_unknown_count(ambiguity_ids) do
        {:error,
         {:underdetermined,
          dual_l1_direct_row_count(dual_epochs, reference_sat, physical_sats, wide_lane_cycles),
          baseline_unknown_count(ambiguity_ids)}}
      else
        state = %{
          baseline: initial_baseline,
        ...
        with {:ok, float_state, float_rows, covariance_cycles, iterations, converged?, status} <-
               iterate_dual_l1_direct_float(
                 base,
                 dual_epochs,
                 reference_sat,
                 physical_sats,
             ...
             {:ok, fixed_cycles, fixed_meta} <-
               IntegerLeastSquares.search(float_state.ambiguities, covariance_cycles, integer_opts),
             {:ok, fixed_state, fixed_rows, fixed_iterations, fixed_converged?, fixed_status} <-
               iterate_dual_l1_direct_fixed(
                 base,
                 dual_epochs,
                 reference_sat,
                 physical_sats,
  ...
  defp dual_l1_direct_ambiguity_index(dual_epochs, reference_sat, wide_lane_cycles) do
    dual_epochs
    |> Enum.reduce_while({:ok, %{}, %{}}, fn epoch, {:ok, ambiguity_satellites, params_acc} ->
      ref_sd = dual_single_difference(epoch, reference_sat)

      epoch
      |> dual_epoch_common_sats()
      |> Enum.reject(&(&1 == reference_sat))
      |> Enum.reduce_while({:ok, ambiguity_satellites, params_acc}, fn sat,
                                                                       {:ok, sat_acc,
                                                                        param_acc} ->
        with {:ok, sample} <- dual_wide_lane_double_difference(epoch, sat, ref_sd),
             {:ok, _wide_lane} <- fetch_dual_wide_lane(wide_lane_cycles, sample.ambiguity_id),
             {:ok, params} <-
               dual_l1_direct_param_from_epoch(epoch, sat, reference_sat, sample.ambiguity_id),
             :ok <-
               ensure_consistent_dual_l1_direct_params(
                 sample.ambiguity_id,
                 params,
                 Map.get(param_acc, sample.ambiguity_id)
               ) do
          case Map.fetch(sat_acc, sample.ambiguity_id) do
            {:ok, ^sat} ->
              {:cont, {:ok, sat_acc, Map.put_new(param_acc, sample.ambiguity_id, params)}}

            {:ok, other_sat} ->
              {:halt, {:error, {:duplicate_ambiguity_id, sample.ambiguity_id, other_sat, sat}}}

            :error ->
              {:cont,
               {:ok, Map.put(sat_acc, sample.ambiguity_id, sat),
                Map.put(param_acc, sample.ambiguity_id, params)}}
          end
        else
          {:error, {:missing_wide_lane_ambiguity, _id}} ->
            {:cont, {:ok, sat_acc, param_acc}}
      ...
      |> case do
        {:ok, sat_acc, param_acc} -> {:cont, {:ok, sat_acc, param_acc}}
        {:error, _reason} = err -> {:halt, err}
      end
    end)
    |> case do
      {:ok, ambiguity_satellites, params} ->
        ambiguity_ids =
          ambiguity_satellites
          |> Enum.sort_by(fn {ambiguity_id, sat} -> {sat, ambiguity_id} end)
          |> Enum.map(&elem(&1, 0))

        physical_sats = ambiguity_satellites |> Map.values() |> Enum.uniq() |> Enum.sort()
        {:ok, ambiguity_ids, Map.new(ambiguity_satellites), physical_sats, params}
  ...
  defp dual_l1_direct_param_from_epoch(epoch, sat, reference_sat, ambiguity_id) do
    sat_base = Map.fetch!(epoch.base, sat)
    sat_rover = Map.fetch!(epoch.rover, sat)
    ref_base = Map.fetch!(epoch.base, reference_sat)
    ref_rover = Map.fetch!(epoch.rover, reference_sat)
    ...
    with :ok <-
      ...
      {:ok,
       %{
         lambda1_m: c / sat_base.f1_hz,
         lambda2_m: c / sat_base.f2_hz,
         f1_hz: sat_base.f1_hz,
         f2_hz: sat_base.f2_hz
       }}
    end
  end

  defp ensure_consistent_dual_l1_direct_params(_id, _params, nil), do: :ok

  defp ensure_consistent_dual_l1_direct_params(ambiguity_id, params, prev) do
    if same_frequency?(params.f1_hz, prev.f1_hz) and same_frequency?(params.f2_hz, prev.f2_hz) do
      :ok
    else
      {:error, {:inconsistent_frequencies, ambiguity_id}}
  ...
  defp dual_l1_direct_row_count(dual_epochs, reference_sat, physical_sats, wide_lane_cycles) do
    dual_epochs
    |> Enum.map(fn epoch ->
      epoch
      |> dual_epoch_common_sats()
      |> Enum.filter(&(&1 in physical_sats))
      |> Enum.reject(&(&1 == reference_sat))
      |> Enum.count(fn sat ->
        ref_sd = dual_single_difference(epoch, reference_sat)
  ...
  defp iterate_dual_l1_direct_float(
         base,
         dual_epochs,
         reference_sat,
         physical_sats,
    ...
    with {:ok, rows} <-
         ...
         {:ok, dx} <- solve_baseline_normal_equations(rows, baseline_unknown_count(ambiguity_ids)) do
      ...
      cond do
        baseline_step <= opts.position_tolerance_m and
          ...
          with {:ok, final_rows} <-
            ...
            {:ok, next, final_rows, covariance_cycles, iter, true, :state_tolerance}
          end

        iter >= opts.max_iterations ->
          with {:ok, covariance_cycles} <-
                 baseline_ambiguity_covariance(
                   rows,
                   baseline_unknown_count(ambiguity_ids),
                   3,
            ...
            {:ok, next, rows, covariance_cycles, iter, false, :max_iterations}
          end
  ...
  defp iterate_dual_l1_direct_fixed(
         base,
         dual_epochs,
         reference_sat,
         physical_sats,
    ...
    with {:ok, rows} <-
           build_dual_l1_direct_rows(
             ...
             physical_sats,
             ambiguity_ids,
             %{state | ambiguities: fixed_cycles},
             weights,
             wide_lane_cycles,
             params,
             :fixed
        ...
        baseline_step <= opts.position_tolerance_m ->
          with {:ok, final_rows} <-
                 build_dual_l1_direct_rows(
                   ...
                   physical_sats,
                   ambiguity_ids,
                   %{next | ambiguities: fixed_cycles},
                   weights,
                   wide_lane_cycles,
                   params,
                   :fixed
                 ) do
            {:ok, next, final_rows, iter, true, :state_tolerance}
          end

        iter >= opts.max_iterations ->
          {:ok, next, rows, iter, false, :max_iterations}
  ...
  defp build_dual_l1_direct_rows(
         base,
         dual_epochs,
         reference_sat,
         physical_sats,
    ...
    dual_epochs
    |> Enum.reduce_while({:ok, []}, fn epoch, {:ok, acc} ->
      ref_sd = dual_l1_direct_single_difference(epoch, reference_sat)
      ...
      |> Enum.filter(&(&1 in physical_sats))
      |> Enum.reject(&(&1 == reference_sat))
      |> Enum.reduce_while({:ok, acc}, fn sat, {:ok, row_acc} ->
        with {:ok, obs_dd} <- dual_l1_direct_double_difference(epoch, sat, ref_sd),
             {:ok, wide_lane} <- fetch_dual_wide_lane(wide_lane_cycles, obs_dd.ambiguity_id) do
          sat_pos = Map.fetch!(epoch.positions, sat)
          {geom_dd, deriv} = geometry_double_difference(base, state.baseline, sat_pos, ref_pos)
          id = obs_dd.ambiguity_id
          param = Map.fetch!(params, id)
          n1 = Map.fetch!(state.ambiguities, id)
          ...
          rows =
            dual_l1_direct_measurement_rows(
              epoch,
              sat,
              id,
              ambiguity_ids,
          ...
          {:cont, {:ok, rows ++ row_acc}}
        else
          {:error, {:missing_wide_lane_ambiguity, _id}} -> {:cont, {:ok, row_acc}}
          {:error, _} = err -> {:halt, err}
        end
      end)
      |> case do
  ...
  defp dual_l1_direct_measurement_rows(
         epoch,
         sat,
         ambiguity_id,
         ambiguity_ids,
    ...
    baseline_h = tuple3_to_list(deriv)

    phase1_h =
      dual_l1_direct_design_row(deriv, ambiguity_id, ambiguity_ids, param.lambda1_m, mode)

    phase2_h =
      dual_l1_direct_design_row(deriv, ambiguity_id, ambiguity_ids, param.lambda2_m, mode)
    ...
    [
      dual_l1_direct_row(
        ...
        ambiguity_id,
        phase2_h,
        obs_dd.l2_m + param.lambda2_m * wide_lane - (geom_dd + param.lambda2_m * n1),
        phase_variance
      ),
      dual_l1_direct_row(
        :phase_l1,
        ...
        ambiguity_id,
        phase1_h,
        obs_dd.l1_m - (geom_dd + param.lambda1_m * n1),
        phase_variance
      ),
      dual_l1_direct_row(
        :code_l2,
  ...
  defp dual_l1_direct_row(kind, epoch, sat, ambiguity_id, h, y, variance) do
    %{
      kind: kind,
      epoch_idx: epoch.idx,
      epoch: epoch.epoch,
      ...
      h: h,
      y: y,
      sd_variance_m2: variance.sd_variance_m2,
      ref_sd_variance_m2: variance.ref_sd_variance_m2,
      weight: variance.weight
    }
  end

  defp dual_l1_direct_design_row(deriv, _ambiguity_id, _ambiguity_ids, _lambda_m, :fixed),
    do: tuple3_to_list(deriv)

  defp dual_l1_direct_design_row(deriv, ambiguity_id, ambiguity_ids, lambda_m, :float) do
    ambiguity_cols = Enum.map(ambiguity_ids, &if(&1 == ambiguity_id, do: lambda_m, else: 0.0))
    tuple3_to_list(deriv) ++ ambiguity_cols
  end

  defp dual_l1_direct_single_difference(epoch, sat) do
    base_obs = Map.fetch!(epoch.base, sat)
    rover_obs = Map.fetch!(epoch.rover, sat)

    lambda1 = Constants.speed_of_light_m_s() / base_obs.f1_hz
    lambda2 = Constants.speed_of_light_m_s() / base_obs.f2_hz

    %{
      satellite_id: sat,
      ambiguity_id: single_difference_ambiguity_id(sat, base_obs, rover_obs),
      p1_m: rover_obs.p1_m - base_obs.p1_m,
      p2_m: rover_obs.p2_m - base_obs.p2_m,
      l1_m: rover_obs.phi1_cyc * lambda1 - base_obs.phi1_cyc * lambda1,
      l2_m: rover_obs.phi2_cyc * lambda2 - base_obs.phi2_cyc * lambda2
    }
  end

  defp dual_l1_direct_double_difference(epoch, sat, ref_sd) do
    sd = dual_l1_direct_single_difference(epoch, sat)
    ...
    {:ok,
     %{
       ...
       reference_satellite_id: ref_sd.satellite_id,
       ambiguity_id: double_difference_ambiguity_id(sat, sd.ambiguity_id, ref_sd),
       p1_m: sd.p1_m - ref_sd.p1_m,
       p2_m: sd.p2_m - ref_sd.p2_m,
       l1_m: sd.l1_m - ref_sd.l1_m,
       l2_m: sd.l2_m - ref_sd.l2_m
     }}
  end

  defp finalize_dual_l1_direct_float(
         base,
         dual_epochs,
         reference_sat,
         physical_sats,
    ...
    covariance_m = l1_cycles_covariance_to_m(covariance_cycles, ambiguity_ids, params)

    with {:ok, covariance_inverse_m} <- invert_matrix(covariance_m) do
      residuals = dual_l1_direct_residuals(rows, reference_sat)
      code_residuals = rows |> Enum.filter(&(&1.kind in [:code_l1, :code_l2])) |> Enum.map(& &1.y)
      phase_residuals = rows |> Enum.filter(&(&1.kind in [:phase_l1, :phase_l2])) |> Enum.map(& &1.y)
      ...
      {:ok,
       %FloatBaselineSolution{
         ...
         ambiguities_m: l1_cycles_to_m(state.ambiguities, params),
         ...
         metadata: %{
           ...
           measurement_covariance: %{
             model: :double_difference_l1_l2_direct,
  ...
  defp finalize_dual_l1_direct_fixed(
         base,
         dual_epochs,
         reference_sat,
         physical_sats,
    ...
    {:ok,
     %FixedBaselineSolution{
       ...
       fixed_ambiguities_m: l1_cycles_to_m(fixed_cycles, params),
       wide_lane_ambiguities_cycles: Map.take(wide_lane_cycles, ambiguity_ids),
       ...
       metadata:
         Map.merge(fixed_meta, %{
           ...
           converged: converged?,
           status: status,
           integer_method: :widelane_l1_direct_lambda,
           wide_lane_fixed: true,
           wide_lane_ambiguities_cycles: Map.take(wide_lane_cycles, ambiguity_ids),
           code_rms_m: rms(code_residuals),
           phase_rms_m: rms(phase_residuals),
  ...
  defp l1_cycles_to_m(cycles, params) do
    Map.new(cycles, fn {id, n1} -> {id, n1 * Map.fetch!(params, id).lambda1_m} end)
  end

  defp l1_cycles_covariance_to_m(covariance_cycles, ambiguity_ids, params) do
    for i <- 0..(length(ambiguity_ids) - 1) do
      id_i = Enum.at(ambiguity_ids, i)
      lambda_i = Map.fetch!(params, id_i).lambda1_m

      for j <- 0..(length(ambiguity_ids) - 1) do
        id_j = Enum.at(ambiguity_ids, j)
        lambda_j = Map.fetch!(params, id_j).lambda1_m
        (covariance_cycles |> Enum.at(i) |> Enum.at(j)) * lambda_i * lambda_j
      end
    end
  end

  defp dual_l1_direct_residuals(rows, reference_sat) do
    rows
    |> Enum.group_by(&{&1.epoch, &1.sat, &1.ambiguity_id})
    |> Enum.map(fn {{epoch, sat, ambiguity_id}, grouped} ->
      code = Enum.find(grouped, &(&1.kind == :code_l1))
      phase = Enum.find(grouped, &(&1.kind == :phase_l1))
