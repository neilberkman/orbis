defmodule Orbis.GNSS.SolutionReport do
  @moduledoc """
  ...
  existing primitive, so each field is traceable to its source:
    ...
    * `residual_m` is copied from `Solution.residuals_m`, paired with
      `Solution.used_sats` by index (the same pairing `Orbis.GNSS.QC.raim/2`
      uses);
    * `normalized_residual` is copied from `Orbis.GNSS.QC.raim/2`'s
      `normalized_residuals` map;
    * `elevation_deg` / `azimuth_deg` come from
      `Orbis.GNSS.Observables.predict/5` evaluated at the **solved** position,
  ...
  ## Options
    ...
    * `:raim` - a keyword list forwarded verbatim to `Orbis.GNSS.QC.raim/2`
