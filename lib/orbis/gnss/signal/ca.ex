defmodule Orbis.GNSS.Signal.CA do
  @moduledoc """
  ...
  @codes Map.new(@phase_select, fn {prn, taps} ->
           {prn, Orbis.GNSS.Signal.CA.Generator.bipolar_code(taps)}
         end)
  ...
  def code_length, do: Constants.ca_code_length()
  ...
  def chip_rate_hz, do: Constants.ca_chip_rate_hz()
  ...
  def chip(prn, index) when is_integer(index) do
    case Map.fetch(@code_tuples, prn) do
      {:ok, chips} ->
        {:ok, elem(chips, Integer.mod(index, Constants.ca_code_length()))}
