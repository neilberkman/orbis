defmodule Orbis.GNSS.Signal.Correlator do
  @moduledoc """
  ...
  def acquire(samples, prn, opts) when is_list(samples) do
    fs = Keyword.get(opts, :sample_rate_hz, @default_sample_rate_hz)
    samples_per_chip = fs / Constants.ca_chip_rate_hz()
    samples_per_code = round(samples_per_chip * Constants.ca_code_length())
    n = length(samples)
      ...
      nil ->
        t =
          Keyword.get(
            opts,
            :integration_time_s,
            Constants.ca_code_length() / Constants.ca_chip_rate_hz()
          )
  ...
  defp sample_code(code, n, fs, code_phase, code_doppler) do
    ...
    code_rate = Constants.ca_chip_rate_hz() * (1.0 + code_doppler / Constants.gps_l1_hz())
