defmodule Orbis.GNSS.DGNSS do
  ...
  and runs `Orbis.GNSS.Positioning.solve/4` with `ionosphere: false` and
    ...
    with {:ok, _base} <- normalize_position(base_position),
      ...
      prc =
        Enum.reduce(base_observations, %{}, fn {sat, pr}, acc ->
          case Observables.predict(source, sat, base_position, epoch,
                 light_time: true,
                 sagnac: true
               ) do
            {:ok, o} ->
              m_base =
                o.geometric_range_m - Constants.speed_of_light_m_s() * (o.sat_clock_s || 0.0)
  ...
  def position(
    ...
    with :ok <- validate_observations(rover_observations, :invalid_rover_observations),
      ...
      case Positioning.solve(source, corrected, epoch, solve_opts) do
        {:ok, solution} ->
