defmodule Orbis.GNSS.ReducedOrbit do
  @moduledoc """
  ...
  **ECEF (ITRF) meters by default**, or GCRS via `frame: :gcrs`. ECEF velocity
  ...
  alias Orbis.GNSS.Core.Validation
  alias Orbis.GNSS.Core.VersionedMap
  alias Orbis.NIF
  ...
  Returns `{:ok, %Orbis.GNSS.ReducedOrbit{}}` or a tagged error:
  ...
  def fit(%Orbis.GNSS.SP3{} = sp3, opts) do
    ...
    with :ok <- require_satellite_id(sat_id),
      ...
      # matching Orbis.GNSS.SP3's contract, so the frame conversion is correct.
      meta = %{
        source: "sp3:#{sat_id}",
  ...
  Returns `{:ok, %Orbis.GNSS.ReducedOrbit{}}` or `{:error, {:unsupported_version, v}}` /
