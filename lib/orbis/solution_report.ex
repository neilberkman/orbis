defmodule Orbis.SolutionReport do
  @moduledoc """
  A consolidated, read-only diagnostic for a single-point-positioning result.

  Given an already-computed `Orbis.PointPositioning.Solution`, an ephemeris
  ...
  existing primitive, so each field is traceable to its source:
    ...
    * `residual_m` is copied from `Solution.residuals_m`, paired with
      `Solution.used_sats` by index (the same pairing `Orbis.GnssQC.raim/2`
      uses);
    * `normalized_residual` is copied from `Orbis.GnssQC.raim/2`'s
      `normalized_residuals` map;
    * `elevation_deg` / `azimuth_deg` come from
      `Orbis.GnssObservables.predict/5` evaluated at the **solved** position,
  ...
  ## Example
      ...
      {:ok, solution} = Orbis.PointPositioning.solve(sp3, observations, epoch)
      {:ok, report} = Orbis.SolutionReport.build(solution, sp3, epoch)

      report.summary.residual_rms_m
      report.summary.integrity.fault_detected?

      report |> Orbis.SolutionReport.format() |> Enum.each(&IO.puts/1)
  """
  ...
  @doc """
  Assemble a `%Orbis.SolutionReport{}` from a solved result, its ephemeris
  ...
  ## Options
    ...
    * `:raim` - a keyword list forwarded verbatim to `Orbis.GnssQC.raim/2`
  ...
  single `Orbis.GnssQC.raim/2` call those options drive, so the report cannot
  diverge from RAIM.
  """
  ...
  def build(%Solution{} = solution, %SP3{} = source, %NaiveDateTime{} = epoch, opts)
    ...
    raim = GnssQC.raim(solution, raim_opts)
  ...
  defp sky(source, sat, position, epoch) do
    case GnssObservables.predict(source, sat, position, epoch) do
