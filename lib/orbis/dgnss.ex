defmodule Orbis.DGNSS do
  ...
  and runs `Orbis.PointPositioning.solve/4` with `ionosphere: false` and
  ...
  `Orbis.GnssObservables.predict/5` is evaluated with `light_time: true` and
  ...
  propagate it, even though `Orbis.GnssObservables` exposes `range_rate_m_s`.
  """
  ...
  # Speed of light in vacuum, m/s (matches the GNSS observables model).
  @c 299_792_458.0
  ...
  `source` is a loaded `Orbis.SP3` product; `base_position` is the known base
  ...
  `Orbis.GnssObservables.predict/5` (light-time and Sagnac on) and the
    ...
    with {:ok, _base} <- normalize_position(base_position),
      ...
      prc =
        Enum.reduce(base_observations, %{}, fn {sat, pr}, acc ->
          case GnssObservables.predict(source, sat, base_position, epoch,
                 light_time: true,
                 sagnac: true
               ) do
            {:ok, o} ->
              m_base = o.geometric_range_m - @c * (o.sat_clock_s || 0.0)
              Map.put(acc, sat, pr / 1.0 - m_base)
  ...
  def apply(rover_observations, corrections)
      when is_list(rover_observations) and is_map(corrections) do
    {corrected, dropped} =
      Enum.reduce(rover_observations, {[], []}, fn {sat, pr}, {ok, drop} ->
        case Map.fetch(corrections, sat) do
          {:ok, prc} -> {[{sat, pr / 1.0 - prc} | ok], drop}
          :error -> {ok, [sat | drop]}
        end
      end)

    {Enum.reverse(corrected), Enum.reverse(dropped)}
  end
  ...
  runs `Orbis.PointPositioning.solve/4` on the corrected pseudoranges with
  ...
  On success returns
      ...
      {:ok, %{
        solution: %Orbis.PointPositioning.Solution{},
  ...
  def position(
      ...
      when is_list(rover_observations) do
    with :ok <- validate_observations(rover_observations, :invalid_rover_observations),
         :ok <- non_empty(rover_observations, :empty_rover_observations),
         {:ok, prc} <- corrections(source, base_position, base_observations, epoch, opts) do
      {corrected, dropped} = __MODULE__.apply(rover_observations, prc)

      solve_opts =
        opts
        |> Keyword.put(:ionosphere, false)
        |> Keyword.put(:troposphere, false)
  ...
  def position(%SP3{}, _base, _base_obs, rover_observations, %NaiveDateTime{}, _opts),
    do: {:error, {:invalid_rover_observations, rover_observations}}
