...
//! `astrodynamics::bodies::sun_moon_ecef` and the tidal displacement by
//! `astrodynamics_gnss::tides::solid_earth_tide`. These entry points decode the
//! Elixir terms, build the crate `TimeScales`, call the crate functions, and
//! encode the results back.

use rustler::{NifResult, Term};

use astrodynamics::bodies::sun_moon_ecef;
use astrodynamics::time::scales::TimeScales;
use astrodynamics_gnss::tides::solid_earth_tide;
