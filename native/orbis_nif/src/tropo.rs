...
fn tropo_zenith_delay(
    ...
    let receiver = Wgs84Geodetic::new(lat_rad, 0.0, height_m);
...
fn tropo_slant_delay(
    ...
    let receiver = Wgs84Geodetic::new(lat_rad, lon_rad, height_m);
    let met = Met::new(pressure_hpa, temperature_k, relative_humidity);
    let epoch =
        Instant::from_julian_date(TimeScale::Gpst, JulianDateSplit::new(jd_whole, jd_fraction));
    Ok(tropo_slant(elevation_rad, receiver, met, epoch))
}
